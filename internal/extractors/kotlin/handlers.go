...
// handleRef renders a file's module documentation and every public declaration
// it carries, each with its KDoc.
func handleRef(
	path string,
	target *string,
	_ []string,
	...
	resolved := resolveKotlinPath(path, sourcePaths, baseDir)
	if resolved == "" {
	...
	if extractors.IsDir(resolved) {
		ktFiles := kotlinFilesIn(resolved)
		if len(ktFiles) == 0 {
			return extractors.FormatError("no .kt files in '" + path + "'"), nil
		}
		for _, kf := range ktFiles {
			content, err := extractors.ReadSource(util.PathJoin(resolved, kf))
			if err != nil {
				content = ""
			}
			files = append(files, sourceFile{name: kf, content: content})
		}
	} else {
...
func handleProseDesc(
	...
	if extractors.IsDir(resolved) {
		for _, kf := range kotlinFilesIn(resolved) {
			content, err := extractors.ReadSource(util.PathJoin(resolved, kf))
			if err != nil || content == "" {
				continue
			}
...
// handleTableSchema renders a data class's fields as a table. A JSON or TOML
// path is a config file rather than Kotlin source, and is rendered as one.
func handleTableSchema(
	...
	dataClasses := extractDataClassFields(source)
	if len(dataClasses) == 0 {
		return extractors.FormatError("no data class types found in '" + path + "'"), nil
	}

	if target != nil && *target != "" {
		for _, class := range dataClasses {
			if class.name == *target {
				return formatDataClassTable(class)
			}
		}
		return extractors.FormatError(
			"data class '" + *target + "' not found in '" + path + "'",
		), nil
	}
	...
	for _, class := range dataClasses {
		...
		table, err := formatDataClassTable(class)
		if err != nil {
			return "", err
		}
